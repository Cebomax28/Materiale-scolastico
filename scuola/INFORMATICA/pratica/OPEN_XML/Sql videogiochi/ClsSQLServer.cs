...
public class ClsSQLServer
{
    ...
    public ClsSQLServer()
    {
        string dbPath = System.IO.Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "db.mdf");
        _connectionString = $@"Data Source=(localdb)\MSSQLLocalDB;AttachDbFilename={dbPath};Initial Catalog=db;Integrated Security=true; Connect Timeout=30";
        _connection = new SqlConnection(_connectionString);
    }

    public ClsSQLServer(string server, string database, string username, string password)
    {
        _connectionString = $@"Data Source={server};Initil Catalog={database};User ID={username};Password={password};Connect Timeout=30";
        _connection = new SqlConnection(_connectionString);
    }
